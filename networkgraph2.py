...
class Self_att(nn.Module):    #自注意得到每个区域、单词的权重
    ...
    def forward(self, x, mask):  #x:n_x,n,d  mask:n_x,1,1,n
        ...
        b = x.size(0)
        n = x.size(1)
        att = att.view(b, n).masked_fill(mask.bool(), -1e9)
...
class Net(nn.Module):
    def __init__(self, arg):
        ...
        self.Rs_GCN_1 = Rs_GCNU(in_channels=512, inter_channels=512)
        self.Rs_GCN_2 = Rs_GCNU(in_channels=512, inter_channels=512)
        self.Rs_GCN_3 = Rs_GCNU(in_channels=512, inter_channels=512)
        self.Rs_GCN_4 = Rs_GCNU(in_channels=512, inter_channels=512)
        self.Rs_GCN_5 = Rs_GCNU(in_channels=512, inter_channels=512)
        self.Rs_GCN_6 = Rs_GCNU(in_channels=512, inter_channels=512)
    ...
    def sim2(self, imgE, txtE, img_mask, txt_mask):
        ...
        imgE3 = self.Rs_GCN_7(imgE2,imgD,txtD)
        txtE3 = self.Rs_GCN_8(txtE2,imgD,txtD)
        imgE4 = self.Rs_GCN_1(imgE3)
        txtE4 = self.Rs_GCN_2(txtE3)
        imgE5 = self.Rs_GCN_3(imgE4)
        txtE5 = self.Rs_GCN_4(txtE4)
        imgE = self.Rs_GCN_5(imgE5)
        txtE = self.Rs_GCN_6(txtE5)
        imgE = l2norm(imgE,2)
        txtE= l2norm(txtE,2)
        ...
        sim_y = sim.masked_fill(y_mask.bool(), -1e9)
        sim_x = sim.transpose(2, 3).masked_fill(x_mask.bool(), -1e9)
