...
class Rs_GCN(nn.Module):
    ...
    def forward(self, v):
        ...
        #R=torch.matmul(l2norm(v.permute(0,2,1),2),l2norm(v.permute(0,2,1),2).permute(0,2,1))  #n_x,n,n

        R = torch.matmul(l2norm(theta_v,2), l2norm(phi_v.permute(0,2,1),2).permute(0,2,1))   #n_x,n,n
        N = R.size(-1)
        R_div_C = R / N
...
class Net(nn.Module):
    ...
    def sim2(self, imgE, txtE, img_mask, txt_mask):
        ...
        n_x = imgE1.size(0)
        n_y = txtE1.size(0)
        imgE2,_ = self.Rs_GCN_1(imgE1)
        txtE2,_ = self.Rs_GCN_2(txtE1)
        imgE3,_ = self.Rs_GCN_3(imgE2)
        txtE3,_ = self.Rs_GCN_4(txtE2)
        imgE4,_ = self.Rs_GCN_5(imgE3)
        txtE4,_ = self.Rs_GCN_6(txtE3)
        ...
        #imgE = self.Rs_GCN_7(imgE4,imgD,txtD)
        #txtE = self.Rs_GCN_8(txtE4,imgD,txtD)
        imgE,aff = self.Rs_GCN_7(imgE4)
        #print(aff.shape)
        g_f = aff[2,3,33]

        txtE,_ = self.Rs_GCN_8(txtE4)
        imgE = imgE.unsqueeze(1).expand(-1, n_y, -1, -1)  # (n_x, n_y, n, d)
        txtE = txtE.unsqueeze(0).expand(n_x, -1, -1, -1)  # (n_x, n_y, m, d)
