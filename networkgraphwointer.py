...
class Rs_GCNI(nn.Module):
    ...
    def forward(self, v,imgD,txtD):
        ...
        imgD=imgD.permute(0,3,2,1)


        g_v = self.g(v).view(batch_size0, self.inter_channels, -1,batch_size1)
        g_v = g_v.permute(0,3,2,1)   #n_x,n_y,n,d
        theta_v = self.theta(imgD).view(batch_size0, self.inter_channels, -1,batch_size1)
        theta_v = theta_v.permute(0,3,2,1)  # n_x,n_y,n,d
        phi_v = self.phi(imgD).view(batch_size0, self.inter_channels, -1,batch_size1)  # n_x,d,n
        phi_v = phi_v.permute(0, 3, 1, 2)  # n_x,n_y,d,n

        R = torch.matmul(l2norm(theta_v, 3), l2norm(phi_v, 3))  # n_x,n_y,n,n
...
class Rs_GCNS(nn.Module):
    ...
    def forward(self,  v,  imgD, txtD):
        ...
        batch_size1 = v.size(1)
        v = v.permute(0, 3,2,1)
        txtD = txtD.permute(0, 3, 2, 1)

        g_v = self.g(v).view(batch_size0, self.inter_channels, -1,batch_size1 )
        g_v = g_v.permute(0, 3,2,1)   #n_x,n_y,m,d
        theta_v = self.theta(txtD).view(batch_size0, self.inter_channels, -1, batch_size1)
        theta_v = theta_v.permute(0, 3, 2, 1)  # n_x,n_y,n,d
        phi_v = self.phi(txtD).view(batch_size0, self.inter_channels, -1, batch_size1)  # n_x,d,n
        phi_v = phi_v.permute(0, 3, 1, 2)  # n_x,n_y,d,n
