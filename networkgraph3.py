...
class Net(nn.Module):
    def __init__(self, arg):
        ...
        self.Rs_GCN_1 = Rs_GCNI(in_channels=512, inter_channels=512)
        self.Rs_GCN_2 = Rs_GCNS(in_channels=512, inter_channels=512)
        self.Rs_GCN_3 = Rs_GCNI(in_channels=512, inter_channels=512)
        self.Rs_GCN_4 = Rs_GCNS(in_channels=512, inter_channels=512)
        self.Rs_GCN_5 = Rs_GCNI(in_channels=512, inter_channels=512)
        self.Rs_GCN_6 = Rs_GCNS(in_channels=512, inter_channels=512)
    ...
    def sim2(self, imgE, txtE, img_mask, txt_mask):
        ...
        imgE2 = imgE1.unsqueeze(1).expand(-1, n_y, -1, -1)  # (n_x, n_y, n, d)
        txtE2 = txtE1.unsqueeze(0).expand(n_x, -1, -1, -1)  # (n_x, n_y, m, d)
        x_mask = img_mask.expand(-1, n_y, -1, -1)  # n_x,n_y,1,n
        y_mask = txt_mask.squeeze(1).unsqueeze(0).expand(n_x, -1, -1, -1)

        imgD, txtD = self.rea(imgE2, txtE2, x_mask, y_mask)
        imgE3 = self.Rs_GCN_7(imgD,imgD,txtD)
        txtE3 = self.Rs_GCN_8(txtD,imgD,txtD)
        imgE4 = self.Rs_GCN_1(imgE3,imgE3,txtD)
        txtE4 = self.Rs_GCN_2(txtE3,imgD,txtE3)
        imgE5 = self.Rs_GCN_3(imgE4,imgE4,txtD)
        txtE5 = self.Rs_GCN_4(txtE4,imgD,txtE4)
        imgE = self.Rs_GCN_5(imgE5,imgE5,txtD)
        txtE = self.Rs_GCN_6(txtE5,imgD,txtE5)
        #imgE3 = self.Rs_GCN_7(imgD, imgD, txtD)
        #txtE3 = self.Rs_GCN_8(txtD, imgD, txtD)
        #imgE4 = self.Rs_GCN_1(imgE3, imgD, txtD)
        #txtE4 = self.Rs_GCN_2(txtE3, imgD, txtD)
        #imgE5 = self.Rs_GCN_3(imgE4, imgD, txtD)
        #txtE5 = self.Rs_GCN_4(txtE4, imgD, txtD)
        #imgE = self.Rs_GCN_5(imgE5, imgD, txtD)
        #txtE = self.Rs_GCN_6(txtE5, imgD, txtD)
